...
def _extract_deadline(text: str, fallback: str = "нужно согласовать") -> str:
    patterns = (
        r"(?:за|в течение)\s+\d+\s*(?:дн|дня|дней|час|часа|часов|недел[яиь])",
        r"\d+\s*-\s*\d+\s*(?:дн|дня|дней|час|часа|часов|недел[яиь])",
        r"(?:срочно|как можно быстрее|без срочности|не срочно)",
    )
    lower_text = text.lower()
...
def _detect_bot_type(text: str) -> str:
    return _pick(
        ...
        [
            ...
            (("недвиж", "квартир", "аренд", "объект"), "Telegram-бот для заявок по недвижимости"),
...
def _detect_goal(text: str) -> str:
    return _pick(
        text,
        [
            (("салон", "барбер", "мастер", "маникюр", "красот"), "помочь клиентам выбирать услугу и записываться на удобное время"),
            (("ресторан", "кафе", "доставк", "еда", "меню"), "показывать меню, принимать заказы и передавать их администратору"),
            (("курс", "школ", "обуч", "урок", "вебинар"), "помогать ученикам получать материалы, оставлять заявки и получать уведомления"),
            (("недвиж", "квартир", "аренд", "объект"), "собирать заявки по объектам и помогать клиентам быстро выбрать подходящий вариант"),
            (("магаз", "товар", "каталог", "корзин"), "помочь клиентам выбирать товары и оформлять заказы внутри Telegram"),
            (("запис", "брон", "слот", "распис"), "автоматизировать запись клиентов и снизить количество ручной переписки"),
            (("заяв", "анкета", "форма", "лид"), "собирать заявки в едином формате и быстро передавать их администратору"),
            ...
            (("парсер", "парсинг", "сбор данных"), "автоматизировать сбор и обработку данных"),
        ],
...
def _detect_modules(text: str) -> list[str]:
    modules = ["стартовое меню с понятной навигацией"]

    if _has_any(text, ("услуг", "о нас", "обо мне", "портфолио", "цены")):
        modules.append("информационные разделы: услуги, цены, описание проекта")
    if _has_any(text, ("ресторан", "кафе", "еда", "меню", "доставк")):
        modules.append("меню/каталог позиций с оформлением заказа")
    if _has_any(text, ("курс", "школ", "обуч", "урок", "материал")):
        modules.append("раздел с материалами, расписанием или заявкой на обучение")
    if _has_any(text, ("недвиж", "квартир", "аренд", "объект")):
        modules.append("подбор объектов и форма заявки на просмотр/консультацию")
    ...
    if _has_any(text, ("админ", "панел", "управ", "добавлять", "редакт")):
    ...
    if _has_any(text, ("оплат", "платеж", "касс", "юкасс", "yookassa")):
    ...
    if _has_any(text, ("файл", "документ", "фото", "картин")):
        modules.append("прием файлов, изображений или документов от пользователя")
...
def _detect_data_fields(text: str) -> list[str]:
    ...
    if not fields:
        fields = ["имя", "контакт для связи внутри Kwork/Telegram", "описание задачи или выбранная услуга"]
...
def _detect_integrations(text: str) -> list[str]:
    ...
    if _has_any(text, ("оплат", "юкасс", "yookassa", "stripe", "платеж")):
...
def _detect_mvp(text: str) -> list[str]:
    ...
    if _has_any(text, ("заяв", "анкета", "форма", "лид", "запис", "магаз", "заказ")):
        mvp.append("уведомление администратору о новой заявке или заказе")
...
def _detect_risks(text: str) -> list[str]:
    risks = []
    if _has_any(text, ("оплат", "платеж", "юкасс")):
    ...
    if _has_any(text, ("crm", "api", "интеграц")):
        risks.append("для интеграций нужны доступы/API-ключи и пример нужного обмена данными")
...
6. Какие данные нужно собирать
{_items(data_fields)}

7. Логика работы пользователя
...
13. Что нужно уточнить перед стартом
{_items(questions)}
""".strip()
